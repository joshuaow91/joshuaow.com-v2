...
const Projects = () => {
    ...
    return (
        <div className="flex flex-col gap-8 mt-10 mb-10">
            <div className="">
                <h2 className="text-2xl text-zinc-300 font-semibold self-start "> <FontAwesomeIcon icon={faBriefcase} className="text-zinc-500 mr-3" />Projects</h2>
            </div>
        
            <div className="flex justify-between items-center gap-8 ">
                <div className="w-2/3 flex flex-col gap-4 border-2 rounded-3xl border-zinc-800 border-r-8 border-b-8 p-4 pl-8 py-8">
                    <h2 className="text-xl text-zinc-300 font-light self-start ">BattleOptions (STAT Analytics) <span className="bg-zinc-800 rounded-full text-xs font-bold text-zinc-500 py-1 px-3 ml-2">Full-Stack</span></h2>
                ...
                >
                    <img 
                        src={dark} 
                        className={`absolute top-0 left-0 h-full border-2 p-1 border-zinc-800 bg-zinc-600 rounded-3xl object-scale-down transition-opacity duration-500 ${isHovered ? 'opacity-0' : 'opacity-100'}`} 
                    />
                    <img 
                        src={light} 
                        className={`absolute top-0 left-0 h-full border-2 p-1 border-zinc-800 bg-zinc-200 rounded-3xl object-scale-down transition-opacity duration-500 ${isHovered ? 'opacity-100' : 'opacity-0'}`} 
                    />
                </div>
            </div>
        </div>
